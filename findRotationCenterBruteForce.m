function c=findRotationCenterBruteForce(a_obj,b_obj,searchradius,resolution)
    c=[];
    %First we guess the center by locating the center of mass of the
    %combined point clouds
    c_guess=0.5*(mean(a_obj.v,1)+mean(b_obj.v,1));
    %Then we look in a circular region around this center for the true
    %rotation center by just trying every location and investigating the
    %Hausdorff distance
